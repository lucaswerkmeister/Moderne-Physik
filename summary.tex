...
\paragraph{Exponentialfunktionen} $e^{ix} = \cos(x) + i \sin(x)$ für reelle $x$.
In QM modelliert man Wellen in Potentialen mit $Ae^{ikx} + Be^{-ikx}$ (Welle) bzw. $Ce^{\kappa x}+De^{-\kappa x}$ (Anstieg/Abfall, Eindringen in Barriere/Wand),
wobei $k,\kappa$ reelle Parameter ($\kappa$ produziert Exponential, $ik$ Welle), $A,B,C,D$ Skalierungs- und Verschiebungsparameter.
\end{multicols}
\end{document}
